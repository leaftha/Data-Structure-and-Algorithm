...
// Searching은 기술적으로는 O(n/2)이지만 실지적으로는 O(n)입니다.

// 정리
// 이중연결 리스트는 prev가 있는 것 빼고는 단일연결리스트와 똑같다.
// 검색 기록을 보는 경우에는 이중연결 리스트를 더 많이 쓴다. prev가 있어
// 앞에서 하거나 뒤로 가거나 해서 시간이 단축된다.
// 하지만 메모리 소모량 더 많아진다.
// 이중연결 리스트는 일정한 상황에서 훨씬 더 좋을 수 있다.
